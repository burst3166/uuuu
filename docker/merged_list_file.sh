...
0,30,31 20-23/1 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_live_redrain.js |ts >> /scripts/logs/jd_live_redrain.log 2>&1
...
0 0-18/6,19-23/1 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_carnivalcity.js |ts >> /scripts/logs/jd_carnivalcity.log 2>&1
##############长期活动##############
# 签到
...
# 京东抽奖机
31 0,12,23 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_lotteryMachine.js |ts >> /scripts/logs/jd_lotteryMachine.log 2>&1
# 京东排行榜
...
# 十元街
12 8,18 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_syj.js |ts >> /scripts/logs/jd_syj.log 2>&1
# 京东快递签到
...
# 领京豆额外奖励(每日可获得3京豆)
23 4 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_bean_home.js |ts >> /scripts/logs/jd_bean_home.log 2>&1
# 微信小程序京东赚赚
6 0,11 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_jdzz.js |ts >> /scripts/logs/jd_jdzz.log 2>&1
# crazyJoy自动每日任务
...
# 京喜app签到
29 7 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jx_sign.js |ts >> /scripts/logs/jx_sign.log 2>&1
# 闪购盲盒
47 8 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_sgmh.js |ts >> /scripts/logs/jd_sgmh.log 2>&1
# 京东秒秒币
10 6 * * * sleep $((RANDOM % $RANDOM_DELAY_MAX)); node /scripts/jd_ms.js |ts >> /scripts/logs/jd_ms.log 2>&1
#美丽研究院
